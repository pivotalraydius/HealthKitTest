...
@property (nonatomic, weak) IBOutlet UILabel *heartRateValueLabel;
@property (nonatomic, weak) IBOutlet UILabel *heartRateUnitLabel;
...
- (NSSet *)dataTypesToWrite {
    ...
    return [NSSet setWithObjects:dietaryCalorieEnergyType, activeEnergyBurnType, heightType, weightType, heartRateType, nil];
}
...
- (void)updateUsersWeightLabel {
    // Fetch the user's default heart rate unit in pounds.
    ...
    // Query to get the user's latest heart rate, if it exists.
...
//    self.weightUnitLabel.text = [NSString stringWithFormat:localizedWeightUnitDescriptionFormat, weightUnitString];
    ...
    [self.healthStore aapl_mostRecentQuantitySampleOfType:heartRateType predicate:nil completion:^(HKQuantity *mostRecentQuantity, NSError *error) {
        if (!mostRecentQuantity) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                self.heartRateValueLabel.text = NSLocalizedString(@"Not available", nil);
            });
        }
        else {
            // Determine the weight in the required unit.
            ...
            double userHeartRate = [mostRecentQuantity doubleValueForUnit:heartUnit];
            
            // Update the user interface.
            dispatch_async(dispatch_get_main_queue(), ^{
                self.heartRateValueLabel.text = [NSNumberFormatter localizedStringFromNumber:@(userHeartRate) numberStyle:NSNumberFormatterNoStyle];
            });
        }
    }];
}
